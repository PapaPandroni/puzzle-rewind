...
- **Full line mode** (Phase 2): instead of a single move, find up to the first 3 of your moves in the engine's refutation line — the app auto-plays the opponent's replies, and on a give-up or after you complete the line you get **Back / Forward step-through controls** to walk the solution move by move at your own pace (rather than the line flashing past automatically). A wrong move rewinds the board to the start of the line so you can try the whole thing again.
...
- **Time controls**: pick which time control the source games come from — Bullet, Blitz, Rapid, Classical, or all of them. Bullet mistakes are often just clock scrambles rather than real misjudgements, so a bullet-heavy account can filter down to its slower, more instructive games. Purely a read-side filter over the accumulated pool (the Lichess fetch always covers every speed), so switching controls is instant and never re-fetches. Bullet covers ultraBullet and Classical covers correspondence; background engine analysis is scoped to the selection too, so the daily budget goes to games the search can actually show.
...
- Interactive board (chessground + chess.js) with legal-move-only drag-and-drop and instant correct/incorrect feedback. A miss isn't fatal: the board rewinds to the starting position and you try again as many times as you like — the solution is only ever revealed by the "give up / show solution" button.
